...
namespace lib
{
    ...
    public class Umover
    {
        ...
        public static bool check(List<bool> _done) //проверка на наличие незаконченных асинхронных методов
        {
        ...
        public async void creator(string[] str, Panel panel, List<Button> umover, List<string> umover_string, List<bool> umover_done) //создание элемента и расположение на элементе panel
        {
            ...
            animations(panel, umover, umover_string, umover_done);
        }
        ...
        public async void animations(Panel panel, List<Button> umover, List<string> umover_string, List<bool> umover_done) //задание конечных значений для элементов
        {
            ...
            for (var i = 0; i < umover.Count; i++)
            {
                ...
                move(umover[i].PointToScreen(Point.Empty), new Point((int)(x - (x / (count + 1.5) * (i + 1))), (int)(y / 2)), umover[i], i, panel,
        ...
        public async void move(Point from, Point to, Button button, int i, Panel panel, List<Button> umover,
            List<string> umover_string, List<bool> umover_done) //движение элементов к конечным значениеям
        {
            umover_done[i] = false;
        ...
        public async Task rdAn(int a, Panel panel, List<Button> umover, List<string> umover_string, List<bool> umover_done, Form form) //удаление элементов и стека
        {
    ...
    public class Deque : Umover
    {
        ...
        public async void animations(Panel panel, List<Button> Deque, List<string> Deque_string, List<bool> Deque_done, bool a)
        {
            ...
            for (var i = Deque.Count - 1; i >= 0; i--)
            {
                ...
                move(Deque[i].PointToScreen(Point.Empty), new Point((int)(x - (x / (count) * (i + 1.5))), (int)(y / 2)), Deque[i], i, panel, Deque, Deque_string, Deque_done);
                await Task.Delay(800);
